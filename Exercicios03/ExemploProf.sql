  ...
  select * from tb_produtos where preco > 20 or categoria_id=1;
   ...
   -- IN e BETWEEN --
    ...
    select * from tb_produtos where preco in (20, 30, 40);
     ...
     select * from tb_produtos where dtvalidade between '2011-11-07' and '2021-11-15';
     ...
     select count(*) from tb_produtos; -- conta os itens --
     
     select count(categoria_id) from tb_produtos;
     
     select sum(preco) from tb_produtos; -- soma preços --
     
     select categoria_id, avg(preco) from tb_produtos group by categoria_id; -- media preços --
     
     select max(preco) as Maior_Preço from tb_produtos; -- Maior preço separado pelo as é apelido --
